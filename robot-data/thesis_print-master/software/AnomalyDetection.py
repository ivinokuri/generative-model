...
def RBA_stage(datasets_preds):
  def apply_rba(rba, type, datasets):
    ...
    for name, dataset in datasets.iteritems():
      pred = rba.transform_same_digits_number(name, type, dataset)
      pred = datasets_preds.intersection_labeling(pred, rba.transform_positive_values(name, type, dataset))
      pred = datasets_preds.intersection_labeling(pred, rba.transform_negative_values(name, type, dataset))
      pred = datasets_preds.intersection_labeling(pred, rba.transform_not_negative_values(name, type, dataset))
      pred = datasets_preds.intersection_labeling(pred, rba.transform_coverage_percentage_columns(name, type, dataset))
      pred = datasets_preds.intersection_labeling(pred, rba.transform_corresponding_columns(name, type, dataset))
      pred = datasets_preds.intersection_labeling(pred, rba.transform_exactly_one_value(name, type, dataset))
