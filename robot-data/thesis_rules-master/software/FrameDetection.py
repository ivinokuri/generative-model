...
class FrameDetection:
    ...
    def RBA_stage(self, datasets_preds):
        ...
        rba.fit(trainings, percentage=0.02, coverage=0.97)
    ...
    def start(self, positives_dir_path, experiments, test_size, average_limit, negatives_dir_path, new_dir_path):
        ...
        for time_dir in time_dir_paths:
            ...
            for i in range(experiments):
                ...
                self.check_percent(delete, time_str, time_dir, test_size)
            ...
            if min_acc_array:
                ...
                min_accu = 1.1
            self.result += "average: " + str(average) + '\n'
            time_percent[time_dir] = average
            if average >= average_limit:
                self.result += "accuracy array: " + str(min_acc_array) + '\n'
                self.result += "minimum accuracy: " + str(min_accu) + '\n'
                self.next_step(delete, time_str, time_dir, negatives_dir_path+str(time_str), new_dir_path, min_accu)
                break
            array_average_percent = []
            min_acc_array = []
        time_percent = sorted(time_percent.items(), key=lambda x: x[1])
        self.result += "time & average: " + str(time_percent) + '\n'



    def check_percent(self, delete, time_str, time_dir, test_size):
        # global result

        datasets_preds = DS.Datasets(time_dir, [], test_size=test_size, delete=int(delete / float(time_str)))
        datasets_preds, rba_info = self.RBA_stage(datasets_preds)
        self.result += self.results(datasets_preds, 0, rba_info=rba_info) + '\n'


    def next_step(self, delete, time_str, time_dir, negatives_dir_path, new_dir_path, min_accu):
        # global result

        datasets_preds = NEWDS.DatasetsNew(time_dir, negatives_dir_path+"/", new_dir_path, time_str, delete=int(delete / float(time_str)))
        datasets_preds, rba_info = self.RBA_stage(datasets_preds)
        self.result += self.results(datasets_preds, min_accu, rba_info=rba_info) + '\n'


    def predictions_information(self, datasets_preds, min_accu):
        def get_acc_info(title, paths_preds, pred_value, max_long, min_accu, r):
            ans = "-----%s-----:" % (title,)
            tmp = ""
            longests = 0.0
            anomalies = 0.0
            ...
            for f, y_pred in paths_preds:
                ...
                for i, line in enumerate(y_pred):
                    ...
                    counter += 1
                    if counter == 10 or i == len(y_pred) - 1:
                        if number_of_zero > max_number_of_zeros:
                            max_number_of_zeros = number_of_zero
                        number_of_zero = 0
                        counter = 0
                # if most_negetive < max_number_of_zeros:
                #     most_negetive = max_number_of_zeros
            ...
            if title == 'positive sets':
                r.faulty_runs.append(faulty_runs)
        ...
        res += get_acc_info("training sets", zip(trainings_names, trainings_preds), Conf.POSITIVE_LABEL, max_long, min_accu, r) + "\n"
        res += get_acc_info("positive sets", zip(positives_names, positives_preds), Conf.POSITIVE_LABEL, max_long, min_accu, r) + "\n"
        if negatives_names:
            res += get_acc_info("negative sets", zip(negatives_names, negatives_preds), Conf.POSITIVE_LABEL, max_long, min_accu, r) + "\n"
        # print r.faulty_runs
        return res


    def results(self, datasets_preds, min_accu, rba_info):
        info = self.predictions_information(datasets_preds, min_accu)
...
if __name__ == '__main__':
    ...
    options, _ = frame_detection.ApplyOptions("gripping_dynamic")
    frame_detection.start(options.positives, options.experiments, options.test_size, options.average_limit, options.negatives, options.new_files)

    frame_detection.result += parse_results.output_to_csv(frame_detection.result)

    print frame_detection.result

    title = None
    name = None
    time = None

    lines = frame_detection.result.split('\n')
    ...
    for line in lines:
        if line.startswith('available runs/time '):
            ...
            time = line_split.split('/')[2] + "_" + line_split.split('/')[3]

    file_path = open(title+"_"+name+"_"+time+".txt", "w")
    file_path.write(frame_detection.result)
