...
class DatasetsNew:
    def __init__(self, positives_dir_paths, negatives_dir_path, new_dir_path, res, delete = 10): # (string, string, double, int)
        trainings_paths = []
        files = DatasetsNew.get_files_name(positives_dir_paths)
        trainings_paths.extend(files)

        positives_paths = []
        positives_dir_paths = DatasetsNew.get_dirs_names(new_dir_path)
        for pos_dir_time in positives_dir_paths:
            positives_dir_time = DatasetsNew.get_dirs_names(pos_dir_time)
            for pos_dir_time in positives_dir_time:
                time_dir_temp = pos_dir_time.rsplit('/', 1)[0]
                time_str = time_dir_temp.rsplit('/', 1)[1]
                if time_str == res:
                    files = DatasetsNew.get_files_name(pos_dir_time)
                    positives_paths.extend(files)
        if not positives_paths:
            print "pos_dir_time: empty"

        negatives_paths = []
        # for neg_dir_path in negatives_dir_path:
        files = DatasetsNew.get_files_name(negatives_dir_path)
        negatives_paths.extend(files)
        if not negatives_paths:
            print "negatives_paths: empty"

        trainings, positives, negatives = DatasetsNew.read_csv_from_paths(trainings_paths, positives_paths, negatives_paths)
        trainings, positives, negatives = DatasetsNew.remove_rows_from(delete, trainings, positives, negatives)
    ...
    def get_names(self):
        trainings_names = map(lambda p: DatasetsNew.__get_filename_from_path(p), self.trainings_preds.paths)
        positives_names = map(lambda p: DatasetsNew.__get_filename_from_path(p), self.positives_preds.paths)
        negatives_names = map(lambda p: DatasetsNew.__get_filename_from_path(p), self.negatives_preds.paths)
    ...
    def __str__(self):
        ...
        def create_info(title, paths, results):
            ...
            #information += "%s files: %s\n" % (title, str(map(DatasetsNew.__get_filename_from_path, paths)))
